...
if(isset($_GET['restore'])){
 ...
 $time = date("Y/m/d H:i:s",strtotime("+0 HOURS"));
  ...
  $row_audit = mysqli_query($conn, "SELECT * FROM `products` WHERE `id` = '$restore_id'") or die('query failed');
   $row_result = mysqli_fetch_assoc($row_audit); 
...
<section class="add-products">
   ...
   <h1 class="title">Archive Products</h1>
...
<section>
   ...
   <?php
         $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE archive ='1'") or die('query failed');
         if(mysqli_num_rows($select_products) > 0){
   ?>
<table class="table">
 <tr class="tr">
 ...
 <td class="td">
           <a href="admin_archive_products.php?delete=<?php echo $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('delete this product?');">delete</a>
         <a style="background-color: green;" href="admin_archive_products.php?restore=<?php echo $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('Restore this product?');">Restore</a>    </td>
