...
if(isset($_GET['delete'])){
   ...
   header('location:admin_walkinorders.php');
}
...
<head>
   ...
   <title>Employee Walk In Orders</title>
...
<!--<section class="orders">
   ...
   <div class="box-container">
      <?php
      $select_orders = mysqli_query($conn, "SELECT * FROM `orders` WHERE payment_status IN ('preparing', 'pending', 'to deliver', 'completed')") or die('query failed');
...
<section>
<h1 class="title">Walk In orders</h1>
   <?php
         $select_orders = mysqli_query($conn, "SELECT * FROM `walkin` WHERE payment_status IN ('preparing', 'pending')") or die('query failed');
      if(mysqli_num_rows($select_orders) > 0){
   ?>
...
<thead>
 ...
 <td data-label="Status">  <form action="" method="post">
  <input type="hidden" name="order_id" value="<?php echo $fetch_orders['id']; ?>">
  <?php if ($fetch_orders['payment_status'] != 'cancelled') { ?>
    <select name="update_payment">
      <option value=><?php echo $fetch_orders['payment_status']; ?></option>
