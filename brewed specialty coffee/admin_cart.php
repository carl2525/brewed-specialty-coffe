...
if(isset($_GET['delete_all'])){
   mysqli_query($conn, "DELETE FROM `cart` WHERE admin_id = '$admin_id'") or die('query failed');
   header('location:admin_cart.php');
}
...
if(isset($_POST['order_btn'])){
   ...
   $name = mysqli_real_escape_string($conn, $_SESSION['admin_name']); 
   $cname = $_POST['cname'];
   ...
   $cart_query = mysqli_query($conn, "SELECT * FROM `cart` WHERE admin_id = '$admin_id'") or die('query failed');
   ...
   if($cart_total == 0){
      ...
      if(mysqli_num_rows($order_query) > 0){
         ...
         mysqli_query($conn, "INSERT INTO `walkin`(admin_id, name, cname, onumber , email, method, total_products, total_price, placed_on) VALUES('$admin_id', '$name', '$cname', '$onumber', '$email', '$method', '$total_products', '$cart_total', '$placed_on')") or die('query failed');
...
<head>
   ...
   <title> Admin Checkout Cart</title>
...
<section class="shopping-cart">
   ...
   <div class="box-container">
      <?php
         $grand_total = 0;
         $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE admin_id = '$admin_id'") or die('query failed');
         if(mysqli_num_rows($select_cart) > 0){
            while($fetch_cart = mysqli_fetch_assoc($select_cart)){   
      ?>
      <div class="box">
         <a href="admin_cart.php?delete=<?php echo $fetch_cart['id']; ?>" class="fas fa-times" onclick="return confirm('delete this from cart?');"></a>
   ...
   <div style="margin-top: 2rem; text-align:center;">
      <a href="admin_cart.php?delete_all" class="delete-btn <?php echo ($grand_total > 1)?'':'disabled'; ?>" onclick="return confirm('Delete All Orders?');">delete all</a>
	  <br>
