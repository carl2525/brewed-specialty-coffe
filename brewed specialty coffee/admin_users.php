...
if(isset($_GET['delete'])){
   ...
   mysqli_query($conn, "UPDATE users SET archive = '1' WHERE id = '$delete_id'") or die('query failed');
 }
 else{
  echo '<script>alert("You Dont Have Privilege to Delete this Account")</script>';
 }
...
<div class="box" style="font-size: 18px;">
         <?php 
            $select_users = mysqli_query($conn, "SELECT * FROM `users` WHERE user_type = 'user' AND archive ='0'") or die('query failed');
      ...
      <div class="box" style="font-size: 18px;">
         <?php 
            $select_admins = mysqli_query($conn, "SELECT * FROM `users` WHERE user_type = 'admin' AND archive ='0'") or die('query failed');
 ...
 <div class="box" style="font-size: 18px;">
         <?php 
            $select_admins = mysqli_query($conn, "SELECT * FROM `users` WHERE user_type = 'employee' AND archive ='0'") or die('query failed');
            $number_of_admins = mysqli_num_rows($select_admins);
         ?>
...
<?php
$select_users = mysqli_query($conn, "SELECT * FROM `users` WHERE archive ='0' and user_type!='super admin'") or die('query failed');
?>
<table class="table">
    <tr class="tr">
    ...
    <tr class="tr">
        ...
        <td class="td"> <span style="color:<?php if($fetch_users['user_type'] == 'admin'){ echo 'var(--red)'; } ?>"><?php echo $fetch_users['user_type']; ?></span> </p>
            <a href="admin_users.php?delete=<?php echo $fetch_users['id']; ?>" onclick="return confirm('Archive this Account?');" class="delete-btn">Archive Account</a>
        </td>
    </tr>
    <?php } ?>
</table>
