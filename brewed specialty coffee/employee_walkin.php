...
if(isset($_POST['add_to_cart'])){
   ...
   $check_cart_numbers = mysqli_query($conn, "SELECT * FROM `cart` WHERE name = '$product_name' AND employee_id = '$employee_id'") or die('query failed');

   if(mysqli_num_rows($check_cart_numbers) > 0){
      $message[] = 'Order Already added!';
   }else{
      mysqli_query($conn, "INSERT INTO `cart`(employee_id, name, price, quantity) VALUES('$employee_id', '$product_name', '$product_price', '$product_quantity')") or die('query failed');
      $message[] = 'Order added!';
   }
...
<section class="menu">
         ...
         <div class="wiicons">   
            <?php
               $select_cart_number = mysqli_query($conn, "SELECT * FROM `cart` WHERE employee_id = '$employee_id'") or die('query failed');
               $cart_rows_number = mysqli_num_rows($select_cart_number); 
            ?>
            <a href="employee_cart.php">  <span style="color:brown"> Check Out (<?php echo $cart_rows_number; ?>)</span> <i class="fas fa-shopping-cart" style="color:brown"></i> </a>
         </div>
