...
if(isset($_POST['submit'])){
   ...
   $hash = password_hash($_POST['cpassword'], PASSWORD_DEFAULT, array('cost' => 12));
   ...
   if(mysqli_num_rows($select_users) > 0){
      ...
      if($pass != $cpass){
         $message[] = 'Confirm password not matched!';
          ...
          $result = mysqli_query($conn, "INSERT INTO `users`(name, email, password, user_type, status) VALUES('$name', '$email','$hash', '$user_type', 0)") or die('query failed');
	...
	if($result){
                    ...
                    $mail->isHTML(true);
                    $url="https://".$_SERVER["HTTP_HOST"].dirname($_SERVER["PHP_SELF"]). "/loginadmin.php";
                    $url1="https://".$_SERVER["HTTP_HOST"].dirname($_SERVER["PHP_SELF"]). "/login_employee.php";
                    $mail->Subject="Your verify code";
                    $mail->Body="<p>Dear user, </p> <h3>Your verify OTP code is $otp <br></h3>
                    You can access the login link via this link.<br>
                    <a href='$url'>Admin login link</a><br>
                    <a href='$url1'>Employee login link</a>
                    <br><br>
                    <p>With regrads,</p>
